...
void main(void)
{
	...
	while(1){
		for(i = 0; i < 10; i++) {
			...
			ozrotatepointy(&t, rot.z);
			t.z += zf; 			/* zoom factor */
			ozplotpoint(&t, &p[i]);
		}
		rot.y = (rot.y+1)%360;
		rot.x = (rot.x+2)%360;
		rot.z = ((rot.z+2)%360 + (rot.z+2)%360)%2;
	...
	ldir
		...
		vz_line2(p[3].x + MX, p[3].y + MY, p[4].x + MX, p[4].y + MY,3);
		...
		vz_line2(p[7].x + MX, p[7].y + MY, p[8].x + MX, p[8].y + MY,3);
		vz_line2(p[8].x + MX, p[8].y + MY, p[9].x + MX, p[9].y + MY,3);
		vz_line2(p[9].x + MX, p[9].y + MY, p[0].x + MX, p[0].y + MY,3);
